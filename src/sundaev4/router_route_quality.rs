...
//!   ROUTE_BLUEPRINT scooper-format blueprint (default: test/fixtures/devnet-blueprint-without-traces.json)
...
const BLUEPRINT_PATH: &str = "test/fixtures/devnet-blueprint-without-traces.json";
...
fn route_quality_vs_k() {
    let market_path = env_or(
        "ROUTE_MARKET",
        "../../LEIOS/bench-market/bench/data/market.json",
    );
    let blueprint_path = env_or("ROUTE_BLUEPRINT", BLUEPRINT_PATH);
    let pairs: Vec<String> = env_list(
        "ROUTE_PAIRS",
        "ADA-USDM,ADA-NIGHT,ADA-SNEK,SNEK-USDM,NIGHT-USDCx",
    );
    ...
    let results: Vec<(String, String, Vec<String>)> = std::thread::scope(|scope| {
        let handles: Vec<_> = jobs
            .iter()
            .map(|&(pair, size)| {
                let (blueprint_path, market_path) = (&blueprint_path, &market_path);
                let (ks, done, total) = (&ks, &done, jobs.len());
                scope.spawn(move || {
                    let out = run_job(blueprint_path, market_path, ks, pair, size);
