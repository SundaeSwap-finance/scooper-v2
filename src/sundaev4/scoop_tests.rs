...
mod tests {
    ...
    fn cs_zap_single_asset_evaluates() {
        ...
        for i in 0..zap.swap_deltas.len() {
            ...
            let offered = if i == 0 { BigInt::from(10_000_019) } else { BigInt::from(0) };
            assert!(net <= offered, "asset {i} consumes {net} of {offered} offered");
        }
    ...
    fn cp_zap_swap_splits_the_offer() {
        let env = TestEnv::from_blueprint_file(BLUEPRINT_PATH);
        let pool = make_pool(&env, 0xDD, token_a(), 1_000_000_000, token_e(), 2_000_000_000);
        ...
        assert!(deltas[0] < offered[0], "the swap spends only part of the offer");
    ...
    fn oversized_zap_offer_is_refused_by_name() {
        ...
        let offered = vec![BigInt::from(2_649_386_506i64), BigInt::from(2_929_443_229i64)];
    ...
    fn cs_zap_partial_basket_three_asset_evaluates() {
        ...
        let offered = [BigInt::from(10_000_000), BigInt::from(5_000_000), BigInt::from(0)];
        for i in 0..3 {
