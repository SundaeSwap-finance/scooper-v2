...
pub const PLUTUS_V3_COST_MODEL: &[i64] = &[
    ...
    20744, 32, 25933, 32, 43053543, 10, 53384111, 14333, 10, 43574283, 26308, 10, 16000, 100,
    16000, 100, 962335, 18, 2780678, 6, 442008, 1, 52538055, 3756, 18, 267929, 18, 76433006, 8868,
...
const SCOOPER_SECRET_KEY: &str = "0101010101010101010101010101010101010101010101010101010101010101";
...
pub static TEST_CTX: std::sync::OnceLock<Option<TestConstraintCtx>> = std::sync::OnceLock::new();
...
impl TestEnv {
    ...
    pub fn from_blueprint_file(path: &str) -> Self {
        ...
        let scripts =
            ScriptStore::from_blueprint(&blueprint).expect("ScriptStore::from_blueprint failed");
        ...
        for validator in &blueprint.validators {
            if let Some(code_hex) = &validator.compiled_code {
                ...
                if let Some(reference) = blueprint.references.iter().find(|r| {
                    ...
                    let tx_hash: Hash<32> = tx_hash_bytes.as_slice().into();
                    let input =
...
pub fn with_route_whitelist(order: Arc<SundaeV4Order>, whitelist: &[Ident]) -> Arc<SundaeV4Order> {
    let Some(Some(ctx)) = TEST_CTX.get() else {
        return order;
    };
    let data = PlutusData::Array(MaybeIndefArray::Def(
        whitelist.iter().map(|i| PlutusData::BoundedBytes(i.to_bytes().to_vec().into())).collect(),
    ));
...
pub fn make_settings(_env: &TestEnv, scooper_keyhash: &[u8]) -> SundaeV4Settings {
    ...
    // any token name.
    let settings_nft_policy =
        TEST_CTX.get().and_then(|c| c.as_ref()).map(|c| c.settings_mint.clone()).unwrap_or_else(
