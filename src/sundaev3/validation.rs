...
pub fn validate_order_value(datum: &OrderDatum, value: &Value) -> Result<(), ValueError> {
    ...
    match &datum.action {
        ...
        Order::Swap(gives, _takes) => {
            ...
            let actual_ada = value.get(&ADA_ASSET_CLASS);
            ...
            let actual_amount_of_give_token = value.get(&gives_asset)
        ...
        Order::Deposit((a, b)) => {
            ...
            let actual_b = value.get(&asset_b);
        ...
        Order::Withdrawal(singleton) => {
            ...
            let actual = value.get(&singleton.asset_class());
            ...
            let actual = value.get(&ADA_ASSET_CLASS);
            if actual < expected {
                return Err(ValueError::HasInsufficientTokens {
                    asset: ADA_ASSET_CLASS,
                    expected,
